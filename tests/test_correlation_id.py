...
def test_health_check_with_correlation_id():
    """Test that health check endpoint works and includes correlation ID in response."""  # noqa: E501
...
def test_mockbin_endpoint_with_correlation_id():
    ...
    response = client.get(
        "/mockbin", headers={"X-Correlation-ID": custom_correlation_id}
    )
    # This might fail due to external API, but correlation ID should be preserved  # noqa: E501
    assert "X-Correlation-ID" in response.headers
    assert response.headers["X-Correlation-ID"] == custom_correlation_id
